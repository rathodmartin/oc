...
<xsl:output method="html" version="1.0" encoding="UTF-8" 
	...
	<xsl:template name="wbsSection">
		...
		<xsl:variable name="imagePath" select="concat(/Element/@BackPath, 'images/')"/>
		<xsl:variable name="colSize" select="count(breakdown[@name='Work Breakdown Structure']/columns/column)"/>
		<div class="sectionHeading">
...
title: "<xsl:value-of select="$workBreakdownText"/>", summary: "<xsl:value-of select="$workBreakdownText"/>", 
columns: [<xsl:for-each select="breakdown[@name='Work Breakdown Structure']/columns/column">
